...
from app.models import (
    AttendanceRecord, CampusSettings, ClassSession, SessionCheckIn,
    Student, User, Notification, db
)
...
def session_start():
    ...
    if lat == 0 and lng == 0:
        return jsonify({"error": "GPS coordinates required. Allow location access."}), 400

    # End any existing active session for this teacher
    existing = ClassSession.query.filter_by(teacher_id=u.teacher_id, is_active=True).all()
    for s in existing:
        s.is_active = False
...
def session_end():
    ...
    if session_id:
        ...
        session = ClassSession.query.filter_by(teacher_id=u.teacher_id, is_active=True).first()
    ...
    unique_students = db.session.query(SessionCheckIn.student_id).filter_by(
        session_id=session.id
    ).distinct().count()
    session.total_checkins = unique_students

    # Also mark attendance records for students who checked in
    checked_student_ids = [r[0] for r in db.session.query(SessionCheckIn.student_id).filter_by(
        session_id=session.id, inside_radius=True
    ).distinct().all()]

    for sid in checked_student_ids:
        rec = AttendanceRecord.query.filter_by(
            student_id=sid, course_code=session.course_code, session_date=date.today()
        ).first()
        if not rec:
...
def session_active():
    ...
    sessions = ClassSession.query.filter_by(is_active=True).order_by(ClassSession.started_at.desc()).all()
    ...
    for s in sessions:
        ...
        checked_in = db.session.query(SessionCheckIn.student_id).filter_by(
            session_id=s.id
        ).distinct().count()
        ...
        if u.student_id:
            already_checked = SessionCheckIn.query.filter_by(
                session_id=s.id, student_id=u.student_id, check_type="initial"
            ).first() is not None
...
def session_ping():
    ...
    session = ClassSession.query.get(session_id)
    if not session or not session.is_active:
        return jsonify({"ok": False, "active": False, "reason": "Session ended"})
    ...
    if not inside:
        ...
        teacher_user = User.query.filter_by(teacher_id=session.teacher_id).first()
